...
class Disciple_Tools_Posts
{
    ...
    public static function update_post_user_meta_fields( array $field_settings, int $post_id, array $fields, array $existing_record ){
        ...
        foreach ( $fields as $field_key => $field ) {
            ...
            if ( isset( $field_settings[ $field_key ] ) && ( $field_settings[ $field_key ]["type"] === "text" || $field_settings[ $field_key ]["type"] === "textarea" || $field_settings[ $field_key ]["type"] === "date" || $field_settings[ $field_key ]["type"] === "key_select" ) ) {
     ...
     */
    public static function update_private_post_meta( int $post_id, string $meta_key, string $meta_value, string $field_type = '') {
        global $wpdb;
        dt_write_log('update_private_post_meta');
        ...
        if ( isset( $meta_value["delete"] ) && $meta_value["delete"] == true || $field_type === 'date' && !is_numeric( $meta_value ) ) {
            dt_write_log( 'HEY '. $meta_value );
            //delete user meta
            $delete = $wpdb->query( $wpdb->prepare( "
            DELETE FROM $wpdb->dt_post_user_meta
            WHERE post_id = %d
        ...
        ", array($post_id, $current_user_id, $meta_key) ), ARRAY_A);
            if ( $all_user_meta ) {
                if ( $field_type === 'multi_select') {
                    dt_write_log("PRIVATE META UPDATE");
                    $previous_data = maybe_unserialize( $all_user_meta[0]["meta_value"] );
                    dt_write_log($previous_data);
                    if ( is_array( $previous_data ) ) {
                        if ( in_array($meta_value, $previous_data) ) {
                        //NEED TO HANDLE DELETEs HERE
                        $meta_value = $previous_data;
                        } else {
                            array_push( $previous_data, $meta_value );
                            $meta_value = $previous_data;
                        }
                    } else {
                        if ($previous_data !== '') {
                            $concatted_data = array( $previous_data );
                        }
                        array_push( $concatted_data, $meta_value );

                        $meta_value = $concatted_data;
                    }
                }
                dt_write_log($meta_value);
                $update = $wpdb->update( $wpdb->dt_post_user_meta,
                ...
                if ( $field_type === 'multi_select') {
                    $new_value = array( $meta_value );
                    $meta_value = maybe_serialize( $new_value );
                    dt_write_log($new_value);
                }
                $create = $wpdb->insert( $wpdb->dt_post_user_meta,
                    [
                        "user_id" => $current_user_id,
