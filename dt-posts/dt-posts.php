...
class DT_Posts extends Disciple_Tools_Posts {
    ...
    public static function create_post( string $post_type, array $fields, bool $silent = false, bool $check_permissions = true, $args = array() ){
        ...
        if ( isset( $args['check_for_duplicates'] ) && is_array( $args['check_for_duplicates'] ) && ! empty( $args['check_for_duplicates'] ) ) {
            ...
            if ( ! empty( $duplicate_post_ids ) && count( $duplicate_post_ids ) > 0 ) {
                ...
                self::add_post_comment( $updated_post['post_type'], $updated_post['ID'], $update_comment, 'comment', array( 'comment_date' => $comment_date ), false, $silent );
    ...
    public static function update_post_comment( int $comment_id, string $comment_content, bool $check_permissions = true, string $comment_type = 'comment', array $args = array() ){
    ...
    public static function get_post_comments( string $post_type, int $post_id, bool $check_permissions = true, string $type = 'all', array $args = array() ) {
        ...
        foreach ( $comments as $comment ){
            ...
            if ( $comment->comment_author_url ){
                ...
                $url = get_avatar_url( $comment->user_id, array( 'size' => '16', 'scheme' => 'https' ) );
            }
            $c = array(
                ...
                'comment_reactions' => array_key_exists( $comment->comment_ID, $comments_meta['reactions'] ) ? $comments_meta['reactions'][$comment->comment_ID] : array(),
                'comment_meta' => array_key_exists( $comment->comment_ID, $comments_meta['meta'] ) ? $comments_meta['meta'][$comment->comment_ID] : array(),
            );
            $response_body[] = $c;
        }
        ...
        foreach ( $comments_meta as $meta ) {
            ...
            if ( strpos( $meta->meta_key, 'reaction' ) === 0 ) {
                if ( !array_key_exists( $meta->comment_id, $comments_reactions_dict ) ) {
                    $comments_reactions_dict[$meta->comment_id] = array();
                }
                if ( !array_key_exists( $meta->meta_key, $comments_reactions_dict[$meta->comment_id] ) ) {
                    $comments_reactions_dict[$meta->comment_id][$meta->meta_key] = array();
                }
            ...
            foreach ( $activity as $a ){
                ...
                if ( isset( $a->user_id ) && $a->user_id > 0 ){
                    ...
                    if ( $user ){
                        ...
                        $a->gravatar = get_avatar_url( $user->ID, array( 'size' => '16', 'scheme' => 'https' ) );
                    }
            ...
            foreach ( $activities as &$activity ) {
                if ( isset( $activity->user_id ) && $activity->user_id > 0 ) {
                    ...
                    if ( $user ) {
                        ...
                        $activity->gravatar = get_avatar_url( $user->ID, array( 'size' => '16', 'scheme' => 'https' ) );
                    }
                }
            }
        }
    ...
    public static function post_messaging( string $post_type, int $post_id, array $args = array() ): array {
        if ( !self::can_view( $post_type, $post_id ) ) {
            return new WP_Error( __FUNCTION__, "No permissions to read $post_type with ID $post_id", array( 'status' => 403 ) );
        }
        ...
        if ( $send_method === 'email' && isset( $post['contact_email'] ) ) {
            ...
            $is_sent = true;
            do_action( 'dt_twilio_send', $post_id, 'post', $message, array( 'service' => 'sms' ) );
