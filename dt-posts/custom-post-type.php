...
class Disciple_Tools_Post_Type_Template {
    ...
    public function __construct( string $post_type, string $singular, string $plural ) {
        ...
        add_filter( 'desktop_navbar_menu_options', array( $this, 'add_navigation_links' ), 20 );
        add_filter( 'dt_nav_add_post_menu', array( $this, 'dt_nav_add_post_menu' ), 10, 1 );
        add_filter( 'dt_templates_for_urls', array( $this, 'add_template_for_url' ) );
        add_filter( 'dt_get_post_type_settings', array( $this, 'dt_get_post_type_settings' ), 10, 4 );
        add_filter( 'dt_get_post_type_settings', array( $this, 'dt_get_post_type_settings_after' ), 1000, 4 );
        add_filter( 'dt_registered_post_types', array( $this, 'dt_registered_post_types' ), 10, 1 );
        add_filter( 'dt_details_additional_section_ids', array( $this, 'dt_details_additional_section_ids' ), 10, 2 );
        add_action( 'init', array( $this, 'register_p2p_connections' ), 50, 0 );
        add_filter( 'dt_capabilities', array( $this, 'dt_capabilities' ), 50, 1 );
        add_filter( 'dt_set_roles_and_permissions', array( $this, 'dt_set_roles_and_permissions' ), 11, 1 );
    }
...
//        }
    ...
    public function dt_capabilities( $capabilities ){
        $capabilities['access_' . $this->post_type] = array(
            'source' => $this->plural,
...
//        ];
        $capabilities['create_'  . $this->post_type] = array(
            'source' => $this->plural,
            'label' => 'Create Records',
            'description' => sprintf( 'The user can create %s', $this->plural ),
            'post_type' => $this->post_type,
        );
        $capabilities['view_any_'  . $this->post_type] = array(
            'source' => $this->plural,
            'label' => 'View All',
            'description' => sprintf( 'The user can view any %s', $this->singular ),
            'post_type' => $this->post_type,
        );
        $capabilities['update_any_'  . $this->post_type] = array(
            'source' => $this->plural,
            'label' => 'Update Any',
            'description' => sprintf( 'The user can update any %s', $this->singular ),
            'post_type' => $this->post_type,
        );
        $capabilities['delete_any_'  . $this->post_type] = array(
            'source' => $this->plural,
            'label' => 'Delete Any',
...
//            'post_type' => $this->post_type
//        ];
        $capabilities['list_all_' .$this->post_type ] = array(
            'source' => $this->plural,
            'label' => 'Preview All',
...
add_action( 'after_setup_theme', function () {
    $custom_post_types = get_option( 'dt_custom_post_types', array() );
    $already_registered = apply_filters( 'dt_registered_post_types', array() );
