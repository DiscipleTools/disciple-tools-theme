...
function dt_admin_pages_scripts() {
    wp_enqueue_script( 'dt_shared_scripts', disciple_tools()->admin_js_url . 'dt-shared.js', array( 'jquery' ), filemtime( disciple_tools()->admin_js_path . 'dt-shared.js' ), true );
    wp_register_style( 'dt_admin_css', disciple_tools()->admin_css_url . 'disciple-tools-admin-styles.css', array(), filemtime( disciple_tools()->admin_css_path . 'disciple-tools-admin-styles.css' ) );
...
function dt_people_groups_post_type_scripts() {
    ...
    if ( ( ( 'post.php' === $pagenow || 'post-new.php' === $pagenow || 'edit.php' === $pagenow )
        ...
        wp_enqueue_script( 'dt_peoplegroups_scripts', get_template_directory_uri() . '/dt-people-groups/people-groups.js', array(
            'jquery',
            'jquery-ui-core',
...
function dt_options_scripts() {
    ...
    if ( isset( $_GET['page'] ) && ( in_array( $_GET['page'], $allowed_pages, true ) ) ) {
        ...
        wp_enqueue_script( 'dt_options_script', disciple_tools()->admin_js_url . 'dt-options.js', array(
            'jquery',
            'jquery-ui-core',
            'jquery-ui-sortable',
            'jquery-ui-dialog',
