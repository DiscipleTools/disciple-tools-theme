jQuery(document).ready(function ($) {
  ...
  $("#add_lang_button").click(function (e) {
    e.preventDefault();
    let selectedLangCode = $("#new_lang_select").val();
    let selectedLang = $("#new_lang_select option:selected").text();
    ...
    let tableLangs = {};
    for (let i = 0; i < langkey.length; i++) {
      console.log(langkey.length);
      let currentLangKey = $(langkey[i]);
      let currentDefaultLabel = $(defaultlabel[i]);
      let currentCustomLabel = $(customlabel[i]);
      let currentIsoCode = $(isocode[i]);
      let currentEnabledKey = $(enabledkey[i]);
      let langKeyText = currentLangKey.text();
      if (!tableLangs[langKeyText]) {
        tableLangs[langKeyText] = [];
        tableLangs[langKeyText].push(langKeyText);
        tableLangs[langKeyText].push(currentDefaultLabel.text());
        // tableLangs[langKeyText].push(
        //   currentCustomLabel.children("input").val()
        // );
        tableLangs[langKeyText].push(currentIsoCode.children("input").val());
        tableLangs[langKeyText].push(
          currentEnabledKey.children("input").prop("checked"),
        );
      }
    }

    let lang_exists = "";
    for (var key in tableLangs) {
      if (Object.prototype.hasOwnProperty.call(tableLangs, "key")) {
        if (tableLangs[key].indexOf(selectedLangCode) !== -1) {
          console.log("you shall not pass");
          lang_exists = "true";
          window.alert("This language has already been added");
          break;
        } else {
          console.log("in here");
          lang_exists = "false";
    ...
    //  language_label[<?php echo esc_html( $language_key ) ?>][default]" value="<?php echo esc_html( ( !isset( $dt_global_languages_list[$language_key] ) || ( isset( $dt_global_languages_list[$language_key] ) && $dt_global_languages_list[$language_key]['label'] != $language_option['label'] ) ) ? $language_option['label'] : '' )

    if (lang_exists == "false") {
      let newRow = $("<tr>").append(
        $("<td class='lang_key' id='lang_key'>").text(selectedLangCode),
        $("<td class='default_label' id='default_label'>").text(selectedLang),
        $(
          "<td class='custom_label' id='custom_label'><input type='text' placeholder='Custom Label'>",
        ),
        $(
          "<td class='iso_code' id='iso_code'><input type='text' placeholder='ISO 639-3 code' maxLength='3'>",
        ),
        $("<td class='enabled' id='enabled'><input type='checkbox'>"),
        $(
          "<td class='translation_key' id='translation_key'><button class='button small expand_translation' data-source='lists'><img style='height: 15px; vertical-align: middle' src='http://dt-testing.local/wp-content/themes/disciple-tools-theme/dt-assets/images/languages.svg'></button>",
        ),
      );

      $("#language_table tbody").append(newRow);
    } else {
      console.log("here");
    }
    langkey = "";
