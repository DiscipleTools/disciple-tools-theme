...
if ( !defined( 'DT_FUNCTIONS_READY' ) ) {
            ...
            <select name="locale">
                <?php foreach ( $languages as $language ) { ?>
                    <option value="<?php echo esc_html( $language['language'] ); ?>" <?php selected( $dt_user_locale === $language['language'] ) ?>>
                        <?php echo esc_html( !empty( $language['flag'] ) ? $language['flag'] . ' ' : '' ); ?>
    ...
    if ( !function_exists( 'dt_render_field_icon' ) ) {
        function dt_render_field_icon( $field, $class = 'dt-icon', $default_to_name = false ) {
            ...
            if ( !empty( $field['icon'] ) && strpos( $field['icon'], 'undefined' ) === false ) {
                ...
                <img class="<?php echo esc_html( $class ); ?>" src="<?php echo esc_url( $field['icon'] ) ?>"
                    alt="<?php echo esc_html( $alt_tag ) ?>" width="15" height="15">
        ...
        if ( isset( $fields[$field_key]['type'] ) && !$custom_display && empty( $fields[$field_key]['hidden'] ) ) {
                    ...
                    <?php foreach ( $fields[$field_key]['default'] as $option_key => $option_value ): ?>
                        ...
                        <?php if ( isset( $option_value['deleted'] ) && $option_value['deleted'] === true ) {
                            ...
                            <div style="display: none"
                                id="link-template-<?php echo esc_html( $field_key ) ?>-<?php echo esc_html( $option_key ) ?>">
            ...
            <?php elseif ( $field_type === 'datetime' ): ?>
                <?php $timestamp = $post[$field_key]['timestamp'] ?? '' ?>
                    <div class="<?php echo esc_html( $display_field_id ); ?> input-group dt_date_time_group"
                        data-timestamp="<?php echo esc_html( $timestamp ) ?>">
                        <input id="<?php echo esc_html( $display_field_id ); ?>" class="input-group-field dt_date_picker" type="text"
                            autocomplete="off" <?php echo esc_html( $required_tag ) ?> value="<?php echo esc_html( $timestamp ) ?>" <?php echo esc_html( $disabled ); ?>>

                        <input type="time" class="input-group-field dt_time_picker"
                            id="<?php echo esc_html( $display_field_id ) . '_time_picker'; ?>"
                            data-field-id="<?php echo esc_attr( $display_field_id ) ?>">

                        <div class="input-group-button">
                            <button id="<?php echo esc_html( $display_field_id ); ?>-clear-button" class="button alert clear-date-button"
                                data-inputid="<?php echo esc_html( $display_field_id ); ?>" title="Delete Date" type="button" <?php echo esc_html( $disabled ); ?>>x</button>
                        </div>
                    </div>
            ...
            <?php elseif ( $field_type === 'location_meta' ): ?>
                <?php if ( DT_Mapbox_API::get_key() && empty( $post ) ): // test if Mapbox key is present ?>
                        <div id="mapbox-autocomplete" class="mapbox-autocomplete input-group" data-autosubmit="false">
                            <input id="mapbox-search" type="text" class="input-group-field" name="mapbox_search" placeholder="Search Location"
                                autocomplete="off" dir="auto" <?php echo esc_html( $disabled ); ?> />
                            <div class="input-group-button">
                                ...
                                <button id="mapbox-clear-autocomplete"
                                    class="button alert input-height delete-button-style mapbox-delete-button" style="display:none;"
                                    type="button" <?php echo esc_html( $disabled ); ?>>&times;</button>
                            </div>
            ...
            <?php elseif ( $field_type === 'location' ): ?>
                    <div class="dt_location_grid" data-id="<?php echo esc_html( $field_key ); ?>">
                        ...
                        <div id="<?php echo esc_html( $field_key ); ?>_t" name="form-<?php echo esc_html( $field_key ); ?>"
                                ...
                                <div class="typeahead__field">
                                    <span class="typeahead__query">
                                        <input class="js-typeahead-<?php echo esc_html( $display_field_id ); ?> input-height"
                                            data-field="<?php echo esc_html( $field_key ); ?>" data-field_type="location"
            ...
            <?php elseif ( $field_type === 'user_select' ): ?>
                    <div id="<?php echo esc_html( $field_key ); ?>" class="<?php echo esc_html( $display_field_id ); ?> dt_user_select">
                        ...
                        <div id="<?php echo esc_html( $display_field_id ); ?>_t" name="form-<?php echo esc_html( $display_field_id ); ?>"
                                ...
                                <div class="typeahead__field">
                                    <span class="typeahead__query">
                                        <input class="js-typeahead-<?php echo esc_html( $display_field_id ); ?> input-height" dir="auto"
                                            ...
                                            data-field_type="user_select" data-field="<?php echo esc_html( $field_key ); ?>"
                                            autocomplete="off" <?php echo esc_html( $disabled ); ?>>
                                    </span>
                                    <span class="typeahead__button">
                                        <button type="button"
                                            class="search_<?php echo esc_html( $field_key ); ?> typeahead__image_button input-height"
                                            data-id="<?php echo esc_html( $field_key ); ?>" <?php echo esc_html( $disabled ); ?>>
                                            <img
    ...
    function render_link_field( $field_key, $option_key, $value, $display_field_id, $meta_id, $required_tag, $disabled ) {
        ...
        <div class="input-group">
            <input type="text" class="link-input input-group-field" value="<?php echo esc_html( $value ) ?>"
                data-meta-id="<?php echo esc_html( $meta_id ) ?>" data-field-key="<?php echo esc_html( $display_field_id ) ?>"
                data-type="<?php echo esc_html( $option_key ) ?>" <?php echo esc_html( $required_tag ) ?>         <?php echo esc_html( $disabled ) ?>>
            <div class="input-group-button">
                <button class="button alert delete-button-style input-height link-delete-button delete-button"
                    data-meta-id="<?php echo esc_html( $meta_id ) ?>" data-field-key="<?php echo esc_html( $field_key ) ?>">
                    &times;
                </button>
            </div>
        </div>
