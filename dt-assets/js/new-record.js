  ...
  $("button.add-button").on("click", (e) => {
    ...
    if (fieldType === "link") {
      ...
      $list.append(`<li style="display: flex">
                <input type="text" class="dt-communication-channel" data-field="${window.SHAREDFUNCTIONS.escapeHTML(
                  listClass
                )}" data-${field}-index="${window.SHAREDFUNCTIONS.escapeHTML(
        elementIndex
      )}"/>
                <button class="button clear delete-button new-${window.SHAREDFUNCTIONS.escapeHTML(
                  listClass
                )}" type="button" data-${field}-index="${elementIndex}">
                    <img src="${window.SHAREDFUNCTIONS.escapeHTML(
                      window.wpApiShare.template_dir
                    )}/dt-assets/images/invalid.svg">
                </button>
                <span class="loading-spinner" data-${field}-index="${window.SHAREDFUNCTIONS.escapeHTML(
        elementIndex
      )}" style="margin: 0.5rem;"></span>
              </li>
              <div class="communication-channel-error" data-${field}-index="${window.SHAREDFUNCTIONS.escapeHTML(
        elementIndex
      )}" style="display: none;">
                ...
                <span class="duplicate-ids" data-${field}-index="${window.SHAREDFUNCTIONS.escapeHTML(
        elementIndex
      )}" style="color: #3f729b;"></span>
              </div>`);
    }
  });
        ...
        if (field_type === "connection") {
          ...
          $.typeahead({
            ...
            template: function (query, item) {
              ...
              ${
                item.status_color
                  ? `<span class="status-square" style="background-color: ${window.SHAREDFUNCTIONS.escapeHTML(
                      item.status_color
                    )};">&nbsp;</span>`
                  : ""
              }
              ${
                item.update_needed && item.update_needed > 0
                  ? `<span>
                <img style="height: 12px;" src="${window.SHAREDFUNCTIONS.escapeHTML(
                  window.wpApiShare.template_dir
                )}/dt-assets/images/broken.svg"/>
                <span style="font-size: 14px">${window.SHAREDFUNCTIONS.escapeHTML(
                  item.update_needed
                )}</span>
              </span>`
                  : ""
  ...
  function typeahead_multi_select_init(is_bulk = false, bulk_id = 0) {
    ...
    for (let input of $(".multi_select .typeahead__query input")) {
      ...
      $.typeahead({
        ...
        searchOnFocus: true,
        template: function (query, item) {
          return `<span>${window.SHAREDFUNCTIONS.escapeHTML(
            item.value
          )}</span>`;
        },
        source: source_data,
