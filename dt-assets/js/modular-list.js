  ...
  function renderBulkEditFieldInput(fieldKey, fieldType, container) {
    ...
    if (fieldType === 'share') {
      ...
      requestAnimationFrame(() => {
        ...
        // If we have a previous user ID (from restore), try to pre-select it
        if (previousUserId) {
          // Note: This would require fetching user data, which might be complex
          // For now, we'll just restore the user_id in fieldData
          // The user will need to re-select if they want to see it visually
          if (fieldData) {
            fieldData.shareUserId = previousUserId;
          }
    ...
    if (fieldType === 'follow') {
      ...
      // Initialize toggle value (default to false - not following)
      requestAnimationFrame(() => {
        const toggleComponent = document.getElementById(followToggleId);
        if (toggleComponent) {
          // Set default value to false (not following)
          // dt-toggle component uses 'checked' property
          if (toggleComponent.checked !== undefined) {
            toggleComponent.checked = false;
          } else if (toggleComponent.value !== undefined) {
            // Fallback: some implementations might use value property
            toggleComponent.value = false;
          } else {
            // Set internal input element if component doesn't expose checked directly
          ...
          // Optional: Check current user's follow state if available
