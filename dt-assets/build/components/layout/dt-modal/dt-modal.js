...
import { m as o } from '../../lit-localize-763e4978.js';
import { o as a } from '../../class-map-8d921948.js';
  ...
  class extends r {
    ...
    constructor() {
      super(),
        ...
        this.addEventListener('open', (t) => this._openModal()),
        this.addEventListener('close', (t) => this._closeModal());
    }
    _openModal() {
      (this.isOpen = !0),
    ...
    _dialogHeader(t) {
      return this.hideHeader
        ? e``
        : e`<header><h1 id="modal-field-title">${this.title}</h1><button @click="${this._cancelModal}" class="toggle">${t}</button></header>`;
    }
    _closeModal() {
      (this.isOpen = !1),
    ...
    _triggerClose(t) {
      this.dispatchEvent(new CustomEvent('close', { detail: { action: t } }));
    }
    _dialogClick(t) {
      if ('DIALOG' !== t.target.tagName) return;
      const e = t.target.getBoundingClientRect();
      !1 ===
        (e.top <= t.clientY &&
          t.clientY <= e.top + e.height &&
          e.left <= t.clientX &&
          t.clientX <= e.left + e.width) && this._cancelModal();
    }
    _dialogKeypress(t) {
      'Escape' === t.key && this._cancelModal();
    }
    _helpMore() {
      return this.isHelp
        ? e`<div class="help-more"><h5>${o('Need more help?')}</h5><a class="button small" id="docslink" href="https://disciple.tools/user-docs" target="_blank">${o('Read the documentation')}</a></div>`
        : null;
    }
    firstUpdated() {
