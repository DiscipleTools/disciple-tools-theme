import { i as t, y as o } from '../../lit-element-2409d5fe.js';
...
window.customElements.define(
  ...
  class extends r {
    ...
    get classes() {
      ...
      return (t[`dt-button--${this.context}`] = !0), t;
    }
    constructor() {
    ...
    handleClick(t) {
      if (!this.confirm || confirm(this.confirm))
        if (this.onClick) t.preventDefault(), this.onClick(t);
        else {
    ...
    render() {
      return this.hide
        ? o``
        : this.href
          ? o`<a class="${n(this.classes)}" href="${this.href}" title="${this.title}" type="${this.type}" @click="${() => this.handleClick()}"><div><slot></slot></div></a>`
          : o`<button class="${n(this.classes)}" title="${this.title}" type="${this.type}" @click="${() => this.handleClick()}"><div><slot></slot></div></button>`;
    }
  },
);
