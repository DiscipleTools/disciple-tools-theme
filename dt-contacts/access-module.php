...
class DT_Contacts_Access extends DT_Module_Base
{
    ...
    public function dt_render_field_for_display_template($post, $field_type, $field_key, $required_tag, $display_field_id, $custom_display = false, $fields = [])
    {
        ...
        if (
            isset($post['post_type']) && $post['post_type'] === 'contacts' && $field_key === 'overall_status'
            ...
            <p>
                ...
                <button id="edit-reason" <?php if ($hide_edit_button): ?> style="display: none" <?php endif; ?>><i
                        class="fi-pencil"></i></button>
            </p>
            <div class="reveal" id="paused-contact-modal" data-reveal>
                ...
                <select id="reason-paused-options">
                    ...
                    foreach ($contact_fields['reason_paused']['default'] as $reason_key => $option) {
                        if ($option['label']) {
                            ?>
                            <option value="<?php echo esc_attr($reason_key) ?>" <?php if (($contact['reason_paused']['key'] ?? '') === $reason_key) {
                                     echo 'selected';
                                 } ?>>
                ...
                <select id="reason-unassignable-options">
                    ...
                    foreach ($contact_fields['reason_unassignable']['default'] as $reason_key => $option) {
                        if (isset($option['label'])) {
                            ?>
                            <option value="<?php echo esc_attr($reason_key) ?>" <?php if (($contact['unassignable_paused']['key'] ?? '') === $reason_key) {
                                     echo 'selected';
                                 } ?>>
                                <?php echo esc_html($option['label'] ?? '') ?>
                            </option>
        ...
        if (
            isset($post['post_type']) && $post['post_type'] === 'contacts' && $field_key === 'assigned_to'
                    ...
                    <div class="typeahead__container" style="margin-bottom: 0">
                        <div class="typeahead__field">
                            ...
                            <span class="typeahead__button">
                                <button type="button"
                                    class="<?php echo esc_html($button_class); ?> typeahead__image_button input-height"
                ...
                <div class="reveal" id="assigned_to_user_modal" data-reveal>
                    <section class="small-12 grid-y grid-margin-y cell dispatcher-tile">
                        <div class="cell dt-filter-tabs">
                            <h4 class="section-header"><?php esc_html_e('Assign To', 'disciple_tools'); ?> <span
                                    id="dispatch-tile-loader" style="display: inline-block; margin-left: 10px"
                                    class="loading-spinner"></span></h4>
                                        ...
                                        <div class="grid-x grid-margin-x" style="margin-top:5px">
                                            <div class="medium-4 cell">
                                                <div class="input-group">
                                                    <input id="search-users-filtered" class="input-group-field" type="text"
                                                        placeholder="Multipliers" <?php echo esc_html($disabled); ?>>
                                                    <div class="input-group-button">
                                                        <button type="button" class="button hollow"><i
    ...
    public function dt_record_top_above_details($post_type, $contact)
    {
        if ($post_type === 'contacts' && isset($contact['type']) && $contact['type']['key'] === 'access') {
            ...
            if (
                isset($contact['overall_status']) && $contact['overall_status']['key'] == 'assigned' &&
                ...
                <section class="cell accept-contact" id="accept-contact">
                    <div class="bordered-box detail-notification-box">
                        ...
                        <button class="accept-button button small accept-decline"
                            data-action="accept"><?php esc_html_e('Accept', 'disciple_tools') ?></button>
                        <button class="decline-button button small accept-decline"
                            data-action="decline"><?php esc_html_e('Decline', 'disciple_tools') ?></button>
                    </div>
                </section>
                <?php
            }
