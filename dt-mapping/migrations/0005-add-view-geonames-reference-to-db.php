...
class DT_Mapping_Module_Migration_0005 extends DT_Mapping_Module_Migration {
    ...
    public function up() {
        ...
        $wpdb->query("
            ...
            LEFT JOIN {$wpdb->prefix}dt_geonames_meta as gn ON g.geonameid=gn.geonameid AND gn.meta_key = 'name'
            LEFT JOIN {$wpdb->prefix}dt_geonames_meta as gp ON g.geonameid=gp.geonameid AND gp.meta_key = 'population'
            LEFT JOIN dt_geonames as gcn ON gcn.geonameid=g.country_geonameid
            LEFT JOIN {$wpdb->prefix}dt_geonames_meta as gcna ON gcna.geonameid=g.country_geonameid AND gcna.meta_key = 'name'
            LEFT JOIN dt_geonames as ga1n ON ga1n.geonameid=g.admin1_geonameid
            LEFT JOIN {$wpdb->prefix}dt_geonames_meta as ga1na ON ga1na.geonameid=g.admin1_geonameid AND ga1na.meta_key = 'name'
            LEFT JOIN dt_geonames as ga2n ON ga2n.geonameid=g.admin2_geonameid
            LEFT JOIN {$wpdb->prefix}dt_geonames_meta as ga2na ON ga2na.geonameid=g.admin2_geonameid AND ga2na.meta_key = 'name'
            UNION ALL
            SELECT
                sub.geonameid,
                sub.name,
