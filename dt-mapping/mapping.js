jQuery(document).ready(function() {
  ...
  if('#mapping_point' === window.location.hash) {
    console.log(DRILLDOWNDATA)
    page_mapping_point()
  }
})
 ...
 *
 **********************************************************************************************************************/
window.DRILLDOWN.map_point_drilldown = function( geonameid ) {
  if ( geonameid !== 'top_map_level' ) { // make sure this is not a top level continent or world request
    console.log('map_point_drilldown: geonameid available ' + geonameid )
    DRILLDOWNDATA.settings.current_map = parseInt(geonameid)
    geoname_points( 'map_chart', parseInt(geonameid) )
    data_type_list( 'data-type-list' )

  }
  else { // top_level maps
    console.log('map_point_drilldown: top level ' + geonameid )
    DRILLDOWNDATA.settings.current_map = 'top_map_level'
    top_level_points( 'map_chart' )
    data_type_list( 'data-type-list' )

  }
}
function page_mapping_point() {
  "use strict";
  let chartDiv = jQuery('#chart')
  chartDiv.empty().html(`
        
        <div class="grid-x grid-margin-y">
            <div class="cell medium-6" id="map_point_drilldown"></div>
            <div class="cell medium-6" style="text-align:right;">
               <strong id="section-title" style="font-size:2em;"></strong><br>
                <span id="current_level"></span>
            </div>
  ...
  DRILLDOWNDATA.settings.hide_final_drill_down = true
  // load drill down
  DRILLDOWN.get_drill_down('map_point_drilldown')

}


function top_level_points( div ) {
  am4core.useTheme(am4themes_animated);
  let chart = am4core.create( div, am4maps.MapChart);
  chart.projection = new am4maps.projections.Miller(); // Set projection
...
function geoname_points( div, geonameid ) {
  am4core.useTheme(am4themes_animated);
