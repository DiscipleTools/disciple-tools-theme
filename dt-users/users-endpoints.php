...
class Disciple_Tools_Users_Endpoints
{
    ...
    public function add_api_routes() {
        register_rest_route(
            $this->namespace, '/users/get_users', array(
                'methods'  => 'GET',
        ...
        register_rest_route(
            $this->namespace, '/users/switch_preference', array(
                'methods'  => WP_REST_Server::CREATABLE,
                'callback' => array( $this, 'switch_preference' ),
                'permission_callback' => '__return_true',
            )
        );

        register_rest_route(
            $this->namespace, '/users/app_switch', array(
                'methods'  => WP_REST_Server::CREATABLE,
                'callback' => array( $this, 'app_switch' ),
                'permission_callback' => '__return_true',
            )
        );

        register_rest_route(
            $this->namespace, '/users/get_filters', array(
                'methods' => 'GET',
        ...
        register_rest_route(
            $this->namespace, '/users/save_filters', array(
                'methods' => 'POST',
                'callback' => array( $this, 'save_user_filter' ),
                'permission_callback' => '__return_true',
            )
        );
        register_rest_route(
            $this->namespace, '/users/save_filters', array(
                'methods' => 'DELETE',
                'callback' => array( $this, 'delete_user_filter' ),
                'permission_callback' => '__return_true',
            )
        );
        register_rest_route(
            $this->namespace, '/users/change_password', array(
                'methods' => 'POST',
        ...
        register_rest_route(
            $this->namespace, '/users/create', array(
                'methods' => 'POST',
        ...
        register_rest_route(
            $this->namespace, '/users/contact-id', array(
                'methods' => 'GET',
                'callback' => array( $this, 'get_user_contact_id' ),
                'permission_callback' => '__return_true',
            )
        );
        register_rest_route(
            $this->namespace, '/users/current_locations', array(
                'methods' => 'GET',
        ...
        register_rest_route(
            $this->namespace, '/users/user_location', array(
                'methods' => 'POST',
                'callback' => array( $this, 'add_user_location' ),
                'permission_callback' => '__return_true',
            )
        );
        register_rest_route(
            $this->namespace, '/users/user_location', array(
                'methods' => 'DELETE',
                'callback' => array( $this, 'delete_user_location' ),
                'permission_callback' => '__return_true',
            )
        );
        register_rest_route(
            $this->namespace, '/user/update', array(
                'methods' => 'POST',
        ...
        register_rest_route(
            $this->namespace, '/user/my', array(
                'methods' => 'GET',
                'callback' => array( $this, 'get_my_info' ),
                'permission_callback' => '__return_true',
            )
        );
    }
    ...
    public function switch_preference( WP_REST_Request $request ) {
        ...
        if ( isset( $params['preference_key'] ) && $user_id ) {
            ...
            if ( $result['status'] ) {
                ...
                return new WP_Error( 'changed_notification_error', $result['message'], array( 'status' => 400 ) );
            }
        } else {
            return new WP_Error( 'preference_error', 'Please provide a valid preference to change for user', array( 'status' => 400 ) );
        }
    }
    ...
    public function app_switch( WP_REST_Request $request ) {
        ...
        if ( isset( $params['app_key'] ) && ! empty( $params['app_key'] ) && $user_id ) {
            ...
            if ( $result['status'] ) {
                ...
                return new WP_Error( __METHOD__, $result['message'], array( 'status' => 400 ) );
            }
        } else {
            return new WP_Error( 'preference_error', 'Please provide a valid preference to change for user', array( 'status' => 400 ) );
        }
    ...
    public function save_user_filter( WP_REST_Request $request ){
        ...
        if ( isset( $params['filter'], $params['post_type'] ) ){
            ...
            return new WP_Error( 'missing_error', 'Missing filters', array( 'status' => 400 ) );
        }
    }
    ...
    public function create_user( WP_REST_Request $request ){
        ...
        if ( isset( $params['user-email'], $params['user-display'] ) ){
            ...
            if ( isset( $params['user-user_role'] ) ){
                $user_roles = array( $params['user-user_role'] );
            }
            ...
            return new WP_Error( 'missing_error', 'Missing fields', array( 'status' => 400 ) );
        }
    }
    ...
    public function add_user_location( WP_REST_Request $request ) {
        ...
        if ( isset( $params['user_location']['location_grid_meta'] ) ) {
            ...
            if ( !Disciple_Tools_Users::can_update( $user_id ) ){
                return new WP_Error( __METHOD__, 'No permission to edit this user', array( 'status' => 400 ) );
            }

            $new_location_grid_meta = array();
    ...
    public function update_user( WP_REST_Request $request ){
        ...
        if ( !$user ) {
            return new WP_Error( 'update_user', 'Something went wrong. Are you a user?', array( 'status' => 400 ) );
        }
    ...
    public function get_my_info( WP_REST_Request $request ) {
        ...
        if ( isset( $user->ID ) && !empty( $user->ID ) ) {
            ...
            foreach ( Disciple_Tools_Users::get_user_settings( $user ) ?? array() as $key => $section ) {
                $info[ $key ] = $section;
            }
            ...
            return new WP_Error( 'get_my_info', 'Something went wrong. Are you a user?', array( 'status' => 400 ) );
        }
    }
}
