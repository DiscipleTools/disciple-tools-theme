...
jQuery(document).ready(function($) {
  ...
  let options_html = `<button class="button small selected-select-button" data-key="all">
    ${_.escape(window.dt_mapbox_metrics.translations.all)}:
  </button>`
  _.forOwn(window.dt_mapbox_metrics.settings.user_status_options, (option, option_key)=>{
    options_html += `<button class="button small empty-select-button" data-key="${_.escape(option_key)}" style="">
      ${_.escape(option)}
    </button>`
  })
  let split_by_html = `
      <div id="${field_key}" class="border-left map-option-buttons">
        ${_.escape(window.dt_mapbox_metrics.translations.user_status)}:
        ${options_html}
      </div>
    `
  ...
  window.mapbox_library_api.area_map.load_detail_panel =  function ( lng, lat, level ) {
    ...
    .done(details=>{
            ...
            level_list.append(`
              ...
              <div class="cell small-2" data-id="${_.escape(v.grid_meta_id)}">
                <a class="button clear delete-button mapbox-delete-button small float-right" data-user_id="${_.escape(v.user_id)}" data-id="${_.escape(v.grid_meta_id)}" data-level="admin0" data-location="${_.escape(details.admin0_grid_id)}">
                  <img src="${_.escape(window.wpApiShare.template_dir)}/dt-assets/images/invalid.svg" alt="delete">
                </a>
              </div>`)
          })
        }
        level_list.append(`<div class="cell add-user-button"><button class="add-user small expanded button hollow" data-level="admin0" data-location="${_.escape(details.admin0_grid_id)}">
          ${_.escape(window.dt_mapbox_metrics.translations.add_user_to.replace('%s', details.admin0_name))}
        </button></div>`)
      ...
      if ( details.admin1_grid_id ) {
        ...
        level_list.append(`<div class="cell add-user-button"><button class="add-user small expanded button hollow" data-level="admin1" data-location="${_.escape(details.admin1_grid_id)}">
          ${_.escape(window.dt_mapbox_metrics.translations.add_user_to.replace('%s', details.admin1_name))}
        </button></div>`)
      }
      if ( details.admin2_grid_id ) {
        ...
        level_list.append(`<div class="cell add-user-button"><button class="add-user expanded small button hollow" data-level="admin2" data-location="${_.escape(details.admin2_grid_id)}">
          ${_.escape(window.dt_mapbox_metrics.translations.add_user_to.replace('%s', details.admin2_name))}
        </button></div>`)
      }
                ...
                <div class="typeahead__field">
                  ...
                  <span class="typeahead__button">
                    <button type="button" class="search_assigned_to typeahead__image_button input-height" data-id="assigned_to_t">
                      <img src="${_.escape(window.wpApiShare.template_dir)}/dt-assets/images/chevron_down.svg" alt="chevron"/>
                    </button>
                  </span>
                </div>
              </div>
        ...
        jQuery.typeahead({
          ...
          template: function (query, item) {
            return `<div class="assigned-to-row" dir="auto">
              ...
              ${ _.escape(item.status_color) ? `<span class="status-square" style="background-color: ${_.escape(item.status_color)};">&nbsp;</span>` : '' }
