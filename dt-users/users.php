...
class Disciple_Tools_Users
{
        ...
        foreach ( $users as $user ) {
            if ( empty( $post_type ) || user_can( $user, 'access_' . $post_type ) ) {
                $u = array(
                    ...
                    'ID'   => $user->ID,
                    'avatar' => get_avatar_url( $user->ID, array( 'size' => '16', 'scheme' => 'https' ) ),
    ...
    public static function update_user_contact_info() {
        ...
        if ( isset( $_FILES['user_profile_pic'] ) && !empty( $_FILES['user_profile_pic']['tmp_name'] ) && class_exists( 'DT_Storage' ) ){
